...
# ── CABECERA ─────────────────────────────────────────────────────────────────
st.title("📄 Recolector de Documentos")
st.markdown("Busca y descarga documentos académicos a través de repositorios de investigación o rastreando sitios web institucionales.")

tab_api, tab_crawl = st.tabs(["🤖 APIs Educativas", "🕸️ Crawling Web"])
...
if btn_run:
    if not crawl_sources and not api_sources:
        st.warning("Debe configurar al menos un método de búsqueda.")
    else:
